...
mode=2; % Specify the calculation mode. 

mineral='sp'; % Specify the mineral type. 
% Choose from: px ol pl ilm sp
...
if strcmp(mineral,'px')==1
    data=readmatrix('Mare.xlsx','Sheet','Px');
elseif strcmp(mineral,'pl')==1
    data=readmatrix('Mare.xlsx','Sheet','Pl');
elseif strcmp(mineral,'ol')==1
    data=readmatrix('Mare.xlsx','Sheet','Ol');
elseif strcmp(mineral,'ilm')==1
    data=readmatrix('Mare.xlsx','Sheet','Ilm');
elseif strcmp(mineral,'sp')==1
    data=readmatrix('Mare.xlsx','Sheet','Sp');
else
...
if strcmp(mineral,'ilm')==1 || strcmp(mineral,'sp')==1
    data=data(:,5:12);
    data(data == 0) = NaN;
else
    data=data(:,5:15);
...
elseif mode==2
    ...
    if strcmp(mineral,'ilm')==1 || strcmp(mineral,'sp')==1
        test_mean=test_mean(1:8);
        test_std=test_std(1:8);
    end
end
...
elseif strcmp(mineral,'pl')==1
    if mode==2
        Ionic_occup=test_fsp(filtered_samples);
    end
    results=array2table(round([filtered_samples,rowSums,Ionic_occup],3),'VariableNames',...
        {'SiO2','TiO2','Al2O3','Cr2O3','FeO','MnO','MgO','CaO','Na2O','K2O','P2O5','Total',...
...
elseif strcmp(mineral,'ol')==1
    ...
    results=array2table(round([filtered_samples,rowSums,Ionic_occup],3),'VariableNames',...
        ...
        'Fe2', 'Mg', 'Ca', 'Na', 'K', 'sum'});
    writetable(results,['Output_', mineral,'.xlsx']);
elseif strcmp(mineral,'ilm')==1 || strcmp(mineral,'sp')==1
    if mode==2
